    ...
    for i in range(x_plots):
        for j in range(y_plots):
            for k in range(num_classes):
                # print(idx)
                axes[i, j].hist(all_projs[:, idx][y == k], bins="auto", alpha=0.1*(10-k), density=density)
...
def save_degeneracy_eigenvalues(LL, history_root):
    ...
    ax.set_xticklabels(eigenvalues_bin_str, rotation=-80, fontsize=4)
...
def save_history(dataset, model, num_classes, history, data_tag, work_magic_code, magic_code, history_root, time_stamp):
    ...
    if data_tag == "mnist" or data_tag == "cifar":
        ...
        to_plot_cob, to_plot_cob_with_original, x_cob = get_to_plot_change_of_basis(x_train, y_train_cat, P, num_classes, with_original=True)

        plot_cob_with_original(to_plot_cob_with_original, dim, dim, num_classes, history_root)
        plot_cob(to_plot_cob_with_original, dim, dim, num_classes, history_root)
