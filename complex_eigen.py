...
class RaiseDimensionNonQuadratic(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.real = self.add_weight(
            name="real",
            shape=(input_shape[2]+1, self.output_dim-input_shape[2]),
            initializer=initializer,
            trainable=True,
            )
        self.imaginary = self.add_weight(
            name="imaginary",
            shape=(input_shape[2]+1, self.output_dim-input_shape[2]),
            initializer=initializer,
            trainable=True,
    ...
    def call(self, inputs):
        self.kernel = tf.complex(self.real, self.imaginary)
        inputs = tf.complex(inputs[:, 0:1], inputs[:, 1:2])
        ...
        outputs_norm = tf.math.sqrt(tf.math.conj(outputs) * outputs)
        outputs = K.relu(outputs_norm) * outputs / outputs_norm

        real_part = tf.math.real(outputs)
        imaginary_part = tf.math.imag(outputs)
        outputs = K.concatenate([real_part, imaginary_part], axis=1)
...
class ReduceDimensionNonQuadratic(layers.Layer):
    ...
    def call(self, inputs):
        ...
        outputs = tf.matmul(inputs_aux, self.kernel)
        outputs_norm = tf.math.sqrt(tf.math.conj(outputs) * outputs)
        outputs = K.relu(outputs_norm) * outputs / outputs_norm
...
class ReduceDimension(layers.Layer):
    ...
    def call(self, inputs):
        ...
        # kernel = tf.matmul(tf.math.conj(self.kernel), self.kernel)
        kernel = self.kernel

        outputs = tf.math.sqrt(tf.matmul(inputs_aux, kernel))
...
class EigenDistApprox(layers.Layer):
    ...
    def call(self, inputs):
        ...
        return K.squeeze(tf.matmul(inputs, thresh), axis=1)
...
class Proj2Prob(layers.Layer):
    ...
    def call(self, inputs):
        ...
        inputs = tf.complex(inputs[:, 0:1], inputs[:, 1:2])
        outputs = tf.real(tf.math.conj(inputs) * inputs)
...
class Projection(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        initializer1 = keras.initializers.Orthogonal(gain=1)
        # initializer = keras.initializers.Identity(gain=1)
        # initializer2 = keras.initializers.Zeros()
        initializer2 = keras.initializers.Orthogonal()
        ...
        self.imaginary = self.add_weight(name="imaginary",
                                      shape=(input_shape[2], self.output_dim),
                                      #initializer="uniform",
                                      initializer=initializer2,
                                      trainable=True
    ...
    def call(self, inputs):
        ...
        mutual_probs = tf.real(tf.math.conj(mutual_projs) * mutual_projs)
        ...
        inputs_norm_squared = tf.matmul(inputs, tf.linalg.adjoint(inputs))
        # inputs_norm_squared = K.sum(inputs_norm_squared, axis=-1, keepdims=True)
        # inputs_norm_squared = K.reshape(inputs_norm_squared, inputs_shape[0:1]+[1,1])
        # real_part = tf.real(inputs_norm_squared)
        # imaginary_part = tf.imag(inputs_norm_squared)
        # inputs_norm = tf.sqrt(real_part)
        # inputs_norm = tf.complex(inputs_norm, imaginary_part)
        inputs_norm = tf.math.sqrt(inputs_norm_squared)
        inputs = inputs / inputs_norm
