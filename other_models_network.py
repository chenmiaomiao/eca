...
def save_heatmap(cm, model_name, data_tag, num_classes):
	# samples_cls = np.sum(cm, axis=0)
	# cm = cm.T / samples_cls
	# cm = cm.T
...
def compare_all(dataset, num_classes, model, data_tag, work_magic_code, magic_code, time_stamp, is_bin=False):
	...
	# if data_tag == "mnist":
	save_heatmap(cm, "eca", data_tag, num_classes)
