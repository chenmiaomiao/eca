...
def save_history(dataset, model, num_classes, history, data_tag, work_magic_code, magic_code, history_root, time_stamp):
    ...
    P2_save_path = os.path.join(history_root, "P2.npy")
    PP2_save_path = os.path.join(history_root, "PP2.npy")
    L2_save_path = os.path.join(history_root, "L2.npy")
    LL2_save_path = os.path.join(history_root, "LL2.npy")
    ReDO_save_path = os.path.join(history_root, "ReDO.npy")
    ...
    L = model.layers[7].get_weights()[0]
    LL = 1/(1+np.exp(np.sin(L) * -GATE_FACTOR))
    print(L)
    print(LL)
    # print(np2latex(LL))

    P2 = model.layers[4].get_weights()[0]
    PP2 = P2.T.dot(P2)
    L2 = model.layers[8].get_weights()[0]
    LL2 = 1/(1+np.exp(np.sin(L2) * -GATE_FACTOR))

    try:
        ReDO = model.layers[3].get_weights()[0]
    except:
        ReDO = np.array([])
    ...
    np.save(P2_save_path, P2)
    np.save(PP2_save_path, PP2)
    np.save(L2_save_path, L2)
    np.save(LL2_save_path, LL2)
    np.save(ReDO_save_path, ReDO)
